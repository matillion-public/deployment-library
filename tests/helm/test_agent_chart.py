...
class TestAgentChart:
    ...
    def base_values(self):
        return {
            ...
            'dpcAgent': {
                ...
                'metricsExporter': {
                    'image': {
                        'repository': 'test/metrics-exporter',
                        'tag': 'v1.0.0'
                    }
                }
            },
    ...
    def test_deployment_has_main_container(self, base_values):
        ...
        # Should have 2 containers: main agent + metrics sidecar
    ...
    def test_deployment_has_metrics_sidecar(self, base_values):
        """Test that deployment has the metrics exporter sidecar"""
        documents = self.helm_template(base_values)
        deployment = self.find_document_by_kind(documents, 'Deployment')
        ...
        assert sidecar['name'] == 'metrics-exporter'
        assert sidecar['image'] == 'test/metrics-exporter:v1.0.0'
        assert sidecar['ports'][0]['containerPort'] == 8000
        assert sidecar['ports'][0]['name'] == 'metrics'

    def test_prometheus_annotations(self, base_values):
        """Test Prometheus scraping annotations are correct"""
        documents = self.helm_template(base_values)
        deployment = self.find_document_by_kind(documents, 'Deployment')
    ...
    def test_resource_configuration(self, base_values):
        """Test resource limits and requests are properly configured"""
        # Add custom resources
        base_values['dpcAgent']['metricsExporter']['resources'] = {
    ...
    def test_image_pull_policy(self, base_values):
        """Test image pull policies are set to Always for security"""
        documents = self.helm_template(base_values)
        deployment = self.find_document_by_kind(documents, 'Deployment')
        
        # Both containers should use Always for security compliance
        main_container = deployment['spec']['template']['spec']['containers'][0]
        sidecar = deployment['spec']['template']['spec']['containers'][1]
