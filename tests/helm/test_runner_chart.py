...
class TestScriptRunner:
    ...
    def test_runner_netpol_ingress_only_from_agent_on_2222(self, base_values):
        ...
        agent = self.find_document_by_kind(documents, 'Deployment', name='-app')
        agent_pod_label = agent['spec']['template']['metadata']['labels']['app']
        assert selector['app'] == agent_pod_label

    def test_runner_tmp_volume_has_size_limit(self, base_values):
        """Runner /tmp emptyDir is capped so a runaway script can't exhaust node
        ephemeral storage (DPC-49794). Default 5Gi, overridable via tmpSizeLimit."""
        documents = self.helm_template(self.enabled_values(base_values))
        runner = next(d for d in self._runner_docs(documents) if d['kind'] == 'Deployment')
        tmp = next(v for v in runner['spec']['template']['spec']['volumes'] if v['name'] == 'tmp-volume')
        assert tmp['emptyDir']['sizeLimit'] == '5Gi'

    def test_runner_tmp_size_limit_override(self, base_values):
        values = self.enabled_values(base_values)
        values['scriptRunner']['tmpSizeLimit'] = '20Gi'
        documents = self.helm_template(values)
        runner = next(d for d in self._runner_docs(documents) if d['kind'] == 'Deployment')
        tmp = next(v for v in runner['spec']['template']['spec']['volumes'] if v['name'] == 'tmp-volume')
        assert tmp['emptyDir']['sizeLimit'] == '20Gi'

    def test_runner_has_tcp_liveness_probe_on_ssh_port(self, base_values):
        """Runner self-heals if sshd dies (DPC-49795 / destructive-test H8) — a TCP
        liveness probe on the SSH listen port lets Kubernetes restart a wedged pod."""
        documents = self.helm_template(self.enabled_values(base_values))
        runner = next(d for d in self._runner_docs(documents) if d['kind'] == 'Deployment')
        container = runner['spec']['template']['spec']['containers'][0]
        probe = container['livenessProbe']
        assert probe['tcpSocket']['port'] == 'ssh'
        # Anchor the defaults so accidental drift is caught.
