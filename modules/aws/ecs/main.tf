locals {
  ...
  agent_cpu    = coalesce(var.agent_cpu, local.agent_size_map[var.agent_size].cpu)
  agent_memory = coalesce(var.agent_memory, local.agent_size_map[var.agent_size].memory)
}
