...
|---|---|---|
| ECS Fargate Terraform — `agent/aws/ecs/terraform.tfvars` | `runner_size` | `small` |
| Azure Container Apps Terraform — `agent/azure/container_apps/terraform.tfvars` | `runner_size` | `small` |
| Helm chart (EKS/AKS/GKE/local) — `values.yaml` or `--set` | `runnerSize` | `small` |
...
Need a different combination — say, 4 vCPU / 30 GiB? Set `runner_cpu` and `runner_memory` directly in `terraform.tfvars`. Both must be set together and the pair must be a valid Fargate combination, otherwise the task definition is rejected at apply time.
...
| `runnerSize` | Pod requests | Recommended node (EKS / AKS / GKE) |
|---|---|---|
...
- **You need a non-standard Fargate combination** — e.g. 2 vCPU / 16 GiB. Set `runner_cpu = 2048` and `runner_memory = 16384` directly.
...
The Terraform overrides take precedence over the size map (set both `runner_cpu` and `runner_memory` together). The Helm override (`dpcAgent.dpcAgent.resources`) replaces the size-derived block whole when non-empty — set both `requests` and `limits`, not just one.
...
If you're upgrading from a pre-`runner_size` checkout:

- **ECS / Container Apps Terraform** — your existing `runner_cpu` / `runner_memory` / `container_cpu` / `container_memory` settings still work as overrides. To switch to the t-shirt model, comment those out and set `runner_size` instead.
- **Helm chart** — old `dpcAgent.dpcAgent.resources` blocks still win as overrides. To switch to the t-shirt model, replace that block with `resources: {}` and set `runnerSize`.
- **Defaults** — `runner_size = "small"` produces 1 vCPU / 4 GiB on every orchestrator, which matches the previous fixed default. Existing deployments that don't set `runner_size` get the same shape they had before.
