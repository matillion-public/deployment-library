#!/usr/bin/env bash
# =============================================================================
# run-check.sh — Matillion Runner Pre-Deployment Check Orchestrator
#
# Auto-discovers the Matillion runner pod using Helm chart labels, performs
...
POD_NAME=""
KUBECONFIG_ARG=""
LABEL_NAME="app.kubernetes.io/name=matillion-runner"
...
Examples:
  ...
  $(basename "$0") -n matillion -r my-runner   # specific release
...
echo ""
box_top
box_line "Matillion Runner Pre-Deployment Check v${VERSION}"
            ...
            if [[ "$ds_name" == *"$pattern"* ]]; then
                ...
                if [[ "$PRODUCT" == "CrowdStrike Falcon" ]]; then
                    ...
                    add_issue "WARN" "CrowdStrike Falcon detected (${ds_name} in ${ds_ns})" \
                        "CrowdStrike's container drift detection can kill Python processes invoked with file arguments (exit 137). If in-pod Python file checks fail, create an exclusion for the Matillion runner container image in the CrowdStrike console."
                fi
...
# --- Auto-discover Matillion runner pod ---
echo ""
...
if [[ -z "$POD_NAME" ]]; then
    ...
    if [[ -z "$POD_DATA" ]]; then
        echo ""
        c_fail "No Matillion runner pods found with selector: ${SELECTOR}"
        detail "Hint: check namespace (--namespace) or release name (--release)"
        detail "Run: kubectl get pods --all-namespaces -l ${LABEL_NAME}"
        add_issue "FAIL" "No runner pods found with selector: ${SELECTOR}" \
            "Ensure the Matillion runner Helm chart is deployed. Verify with: kubectl get pods --all-namespaces -l ${LABEL_NAME}"
        echo ""
        box_top
        box_line "RESULT: FAIL — could not discover runner pod" "${RED}RESULT: FAIL${NC} — could not discover runner pod"
        box_bottom
        exit 1
    fi
    ...
    if [[ -z "$FOUND_POD" ]]; then
        c_fail "Found runner pods but none are in Running state"
        add_issue "FAIL" "No runner pods in Running state" \
        ...
        echo ""
        box_top
        box_line "RESULT: FAIL — no running runner pod" "${RED}RESULT: FAIL${NC} — no running runner pod"
        box_bottom
        exit 1
    fi
    ...
    IFS=$'\t' read -r RUNNER_NODE FOUND_RESTARTS FOUND_PHASE <<< "$KC_OUT"
    FOUND_RESTARTS="${FOUND_RESTARTS:-0}"
    FOUND_PHASE="${FOUND_PHASE:-unknown}"
fi

# --- C5: Pod status and restart count ---
c_info "Runner pod: ${POD_NAME} (${FOUND_PHASE:-unknown}, ${FOUND_RESTARTS:-0} restarts)"
if [[ -n "$FOUND_RESTARTS" && "$FOUND_RESTARTS" =~ ^[0-9]+$ && "$FOUND_RESTARTS" -gt 5 ]]; then
    c_warn "Runner pod has ${FOUND_RESTARTS} restarts — investigate restart reasons"
    add_issue "WARN" "Runner pod has ${FOUND_RESTARTS} restarts" \
        "High restart count indicates recurring failures. Check logs: kubectl logs ${POD_NAME} -n ${NAMESPACE} --previous"
fi

# --- C2: Security DaemonSet pods on runner's node ---
if [[ -n "$RUNNER_NODE" && ${#SECURITY_FOUND[@]} -gt 0 ]]; then
    echo ""
    echo -e "  ${BOLD}--- Security Agents on Node: ${RUNNER_NODE} ---${NC}"
    for entry in "${SECURITY_FOUND[@]}"; do
        ...
        kc_safe "list ${ds_product} pods on node ${RUNNER_NODE}" get pods -n "$ds_ns" \
            --field-selector "spec.nodeName=${RUNNER_NODE}" \
        ...
        if [[ -n "$KC_OUT" ]]; then
            while IFS=$'\t' read -r np_name np_phase; do
                ...
                if [[ "$np_name" == *"$ds_ds_name"* || "$np_name" == *"falcon"* || "$np_name" == *"defender"* || "$np_name" == *"falco"* || "$np_name" == *"sysdig"* || "$np_name" == *"twistlock"* || "$np_name" == *"aqua"* || "$np_name" == *"neuvector"* ]]; then
                    c_warn "${ds_product} pod running on runner node: ${np_name} (${np_phase})"
                fi
            done <<< "$KC_OUT"
        fi
    done
...
if [[ -n "$NAMESPACE" ]]; then
    ...
    if [[ -n "$PSS_ENFORCE" ]]; then
        if [[ "$PSS_ENFORCE" == "restricted" ]]; then
            c_warn "Namespace ${NAMESPACE} has PSS enforce=${PSS_ENFORCE} — may restrict runner execution"
            add_issue "WARN" "Namespace PSS enforce=restricted" \
                "The 'restricted' Pod Security Standard may prevent the runner from running. Consider using 'baseline' or adding an exemption for the runner namespace."
        else
...
if [[ -n "$CONTAINER_IMAGE" ]]; then
    ...
    case "$IMAGE_TAG" in
        current)
            ...
            detail "Support: latest release + one prior. Full SaaS runners always run Current."
        ...
        case "${POD_CLOUD_PROVIDER}" in
            ...
            "")
                ...
                add_issue "WARN" "CLOUD_PROVIDER not set" \
                    "Set the CLOUD_PROVIDER environment variable on the runner pod"
        ...
        if echo "$NP_POLICY_TYPES" | grep -q 'Egress'; then
            if [[ -z "$NP_EGRESS" || "$NP_EGRESS" == "[]" ]]; then
                ...
                add_issue "WARN" "NetworkPolicy '${np_name}' blocks all egress" \
                    "The runner needs egress to port 443 (HTTPS) and port 53 (DNS). Add egress rules or remove the Egress policy type."
            else
                ...
                if [[ "$HAS_DNS" == true && "$HAS_HTTPS" == true ]]; then
                    ...
                    add_issue "WARN" "NetworkPolicy '${np_name}' may block ${MISSING}" \
                        "The runner requires egress to port 443 (Matillion API, keycloak, OTEL) and port 53 (DNS). Add the missing egress rules."
                fi
            fi
        else
