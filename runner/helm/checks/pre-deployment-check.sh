#!/usr/bin/env bash
# =============================================================================
# pre-deployment-check.sh — Matillion Runner In-Pod Validation
#
# Runs inside a Matillion runner pod to validate the runtime environment.
...
echo ""
box_top
box_line "Matillion Runner In-Pod Check v${VERSION}"
    ...
    if [[ -f "$CHECK_SCRIPT" ]]; then
        ...
        if [[ $FILE_EXIT -eq 0 && "$FILE_RESULT" == "matillion_check_ok" ]]; then
            ...
            add_issue "FAIL" "Python3 file-based execution killed (exit 137)" \
                "A runtime security tool (likely CrowdStrike Falcon) is killing Python when invoked with a file argument. Create a container exclusion policy in your security tool for the Matillion runner container/image."
        else
...
if touch /tmp/_matillion_write_test 2>/dev/null; then
    ...
    add_issue "FAIL" "/tmp is not writable" \
        "The runner writes temporary scripts to /tmp. Ensure /tmp is mounted as writable (check volumeMounts in the Deployment spec)."
fi

# --- P6: working directory ---
WORK_DIR="${MTLN_EMERALD_WORKING_DIR:-/home}"
CURRENT_USER=$(whoami 2>/dev/null || echo "unknown")
# The runner writes to subdirectories (e.g. /home/agentuser), not /home itself
...
if command -v java >/dev/null 2>&1; then
    ...
    add_issue "FAIL" "Java not found on PATH" \
        "The Matillion runner is a Spring Boot Java application. Ensure a JDK/JRE is installed in the container image."
fi
...
if [[ -n "$TMP_MOUNT_FLAGS" ]]; then
    if echo "$TMP_MOUNT_FLAGS" | grep -q 'noexec'; then
        ...
        add_issue "WARN" "/tmp is mounted with noexec" \
            "Remove the noexec mount option from /tmp, or mount a separate writable+executable tmpfs for the runner's script directory."
    else
...
# JVM process memory
JAVA_PID=$(pgrep -f 'java.*spring\|java.*emerald\|java.*agent\|java.*runner' 2>/dev/null | head -1)
if [[ -n "$JAVA_PID" ]]; then
...
# Matillion platform endpoints the runner must reach
