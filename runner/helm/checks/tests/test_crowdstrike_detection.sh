...
if [[ "$1" == "get" && "$2" == "pods" ]]; then
    # Check if querying for security pods on node or runner pods
    ...
    # Runner pod query
    printf 'test-ns\ttest-runner-app-abc123\tRunning\tnode-1\t0\n'
    exit 0
fi
if [[ "$1" == "get" && "$2" == "pod" ]]; then
...
assert_output_contains "$CS_OUTPUT" \
    "pod running on runner node" \
    "CrowdStrike pod detected on runner's node"
