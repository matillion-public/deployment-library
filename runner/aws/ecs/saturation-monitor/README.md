# ECS Runner Saturation Monitor - Testing Deployment

This optional deployment creates a Lambda function to monitor ECS runner task saturation and publish metrics to CloudWatch.
...
### Optional Variables
- `cloudwatch_namespace` - CloudWatch namespace (default: "ECS/RunnerSaturation")
- `schedule_expression` - How often to run (default: "rate(1 minute)")
- `log_level` - Lambda logging level (default: "INFO")
- `runner_service_indicators` - Service name patterns to match (default: "matillion,runner,agent,dpc")
...
2. **Update runner_service_indicators:**
   ```hcl
   # In terraform.tfvars
   runner_service_indicators = "your-actual-service-name,another-pattern"
   ```
...
1. **Check runner endpoints are accessible:**
...
2. **Verify actuator endpoint format:**
   ...
   - Ensure your runner exposes this endpoint
...
module "saturation_monitor" {
  source = "./runner/aws/ecs/saturation-monitor"
  
  name                     = var.name
  runner_service_indicators = "matillion,runner,agent,dpc"
  create_alarms           = true
